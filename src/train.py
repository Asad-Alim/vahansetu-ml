# """
# train.py — Batch PPO training with logging and visual output per batch.

# Usage:
#     python src/train.py                         # 3 batches x 30 min
#     python src/train.py --batches 5 --minutes_per_batch 20
...
#     steps_per_batch = max(args.minutes_per_batch * 60 * 3, 2048)
#     print(f"Each batch ≈ {steps_per_batch} steps (~{args.minutes_per_batch} min on CPU)")
...
#     total = args.batches
#     print(f"\nStarting {total} batch(es) x ~{args.minutes_per_batch} min each")

#     for batch_num in range(batch_start, batch_start + total):
#         print(f"\n{'='*60}")
#         print(f"  BATCH {batch_num} / {batch_start + total - 1}  ({steps_per_batch} steps)")
#         print(f"{'='*60}")

#         before  = len(episode_log)
#         t_start = time.time()

#         model.learn(total_timesteps=steps_per_batch,
...
#               f"Episodes: {len(new_eps)} | "
#               f"Avg reward: {avg_reward:.4f} | "
#               f"Improvement: {avg_improv:.1f}% | "
#               f"Violations: {avg_viols:.1f}")
...
#             "batch": batch_num, "avg_reward": round(avg_reward, 4),
#             "avg_improvement": round(avg_improv, 2),
#             "avg_violations": round(avg_viols, 2),
#             "elapsed_min": round(elapsed, 1), "episodes": len(new_eps),
#         })
...
#         print(f"  You can stop now (Ctrl+C) or wait for next batch.")

#         if batch_num < batch_start + total - 1:
#             print(f"\n  [Next batch in 5 sec... Ctrl+C to stop]")
#             try:
#                 time.sleep(5)
...
#     p.add_argument("--minutes_per_batch", type=int, default=30)
#     p.add_argument("--max_files",         type=int, default=80)
