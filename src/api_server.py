...
# ─── Pre-auction (repeat for each filtered courier) ───────────────────────────
#   Backend already stores courier's current total_distance_km / travel_time_min.
#   Call /optimize with  { existing_orders + new_order, use_ppo: false }
#   Backend computes:
#       detour_distance = response.summary.total_distance_km - stored_old_distance
#       detour_time     = response.summary.travel_time_min   - stored_old_time
#   Rank couriers by detour_distance ascending → broadcast top-50.
 
# ─── Post-auction re-optimisation (hours later) ────────────────────────────────
#   Courier may have moved.  Pass current GPS as courier_lat / courier_lon.
#   Call /optimize with  { all_final_orders, use_ppo: true, courier_lat, courier_lon }
#   Route is re-optimised starting from the courier's actual current location.
 
# Run from implementation_3.0/:
...
# # ────────────────────────────────────────────────────────────────────────────
# # PPO model – loaded once at startup
# # ────────────────────────────────────────────────────────────────────────────
# PPO_MODEL_PATH = os.getenv("PPO_MODEL_PATH", "models_v4/ppo_alns_final")
# _ppo_model = None
...
#         print(f"[INFO] PPO model loaded from {PPO_MODEL_PATH}")
#     except Exception as e:
#         print(f"[WARN] PPO model not loaded: {e}. Will use greedy fallback.")
 
# _load_ppo()
 
# app = FastAPI(title="VahanSetu ALNS Microservice", version="1.0.0")
...
#     ready_time:   float
#     due_time:     float
#     service_time: float = 10.0   # minutes
...
#     # Courier's current GPS location.
#     # Pre-auction: omit (centroid of pickups used as start).
#     # Post-auction re-optimisation: pass actual current location.
#     courier_lat: Optional[float] = None
#     courier_lon: Optional[float] = None
 
 
# # ────────────────────────────────────────────────────────────────────────────
# # Converters: Pydantic → internal dataclasses
# # ────────────────────────────────────────────────────────────────────────────
...
#     svc = n.service_time
#     if n.is_pickup:
#         node.packaging_time = svc / 2
#         node.loading_time   = svc / 2
#     else:
#         node.unloading_time = svc
#     return node
...
#     """
#     Build an Instance from the API request.
#     If courier_lat/lon are provided, the route starts from the courier's
#     current location; otherwise centroid of all pickup nodes is used.
#     """
#     int_orders  = [_to_order(o) for o in req.orders]
#     int_vehicle = _to_vehicle(req.vehicle)
 
#     if req.courier_lat is not None and req.courier_lon is not None:
#         start_lat = req.courier_lat
#         start_lon = req.courier_lon
#     else:
#         start_lat = sum(o.pickup_node.lat for o in req.orders) / len(req.orders)
#         start_lon = sum(o.pickup_node.lon for o in req.orders) / len(req.orders)
 
#     start_node = Node(
#         node_id="START", is_pickup=False,
#         x=start_lat, y=start_lon,
#         lat=start_lat, lon=start_lon,
#         district="Current Location",
#         ready_time=0, due_time=1440,
#     )
...
# def _run_greedy(instance: Instance, max_iter: int = 50) -> Solution:
...
# def _run_ppo(instance: Instance, max_iter: int = 50) -> Solution:
#     if _ppo_model is None:
#         print("[INFO] PPO not available, falling back to greedy.")
#         return _run_greedy(instance, max_iter)
...
# # ────────────────────────────────────────────────────────────────────────────
# # Response builder  —  matches your existing output JSON format exactly
# # ────────────────────────────────────────────────────────────────────────────
 
# def _build_response(instance: Instance, solution: Solution, start_time: str) -> dict:
#     orders_map = {o.order_id: o for o in instance.orders}
...
#         solution.route, result.arrival_times, result.departure_times, start_time)
 
#     # enrich each stop with lat/lon
...
#     ) if instance.vehicle.max_weight else 0.0
...
#     """
#     Optimise a courier's route and return full schedule + summary.
 
#     Pre-auction (use_ppo=false, faster):
#       Send existing_orders + new_order.
#       Backend computes detour = new total_distance_km - old total_distance_km.
 
#     Post-auction re-optimisation (use_ppo=true, best quality):
#       Send all final orders + courier_lat + courier_lon.
#       Returns fresh optimised route from courier's current location.
#     """
#     if not req.orders:
...
#     try:
#         solution = _run_ppo(instance, req.max_iter) if req.use_ppo \
#                    else _run_greedy(instance, req.max_iter)
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=f"Optimiser error: {e}")
