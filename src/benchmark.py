...
def run_once(instance, mode: str, ppo_model, max_iter: int, seed: int,
             travel_seed: int = None) -> dict:
    """
    Run one solve (greedy or PPO) on `instance`.
 
    Args:
        instance:     Pre-augmented Instance (vehicle/orders already fixed by
                      the caller so greedy & PPO share identical inputs).
        mode:         "greedy" or "ppo".
        ppo_model:    Loaded PPO model, or None.
        max_iter:     ALNS iteration budget.
        seed:         RNG seed for operator selection / SA decisions.
        travel_seed:  Seed forwarded to reset_travel_cache().  The caller
                      must pass the SAME value for greedy and PPO so both
                      algorithms see identical node-to-node travel times.
                      If None, a fresh unseed reset is used (training mode).
    """
    from data_loader import reset_travel_cache
    # Pin travel times to the shared seed so greedy and PPO see the same
    # speed for every A→B pair.  Cache is cleared first so stale values
    # from the previous solver don't bleed through.
    ...
    for iteration in range(max_iter):
        ...
        if mode == "ppo" and ppo_model is not None:
            ...
            d_usage = destroy_usage / total
            r_usage = repair_usage  / total
            tws = [(o.pickup_node.due_time - o.pickup_node.ready_time) for o in instance.orders]
            avg_tw  = np.mean(tws) / MAX_TW
            svc = [o.pickup_node.service_time + o.delivery_node.service_time for o in instance.orders]
            avg_svc = np.mean(svc) / MAX_SERVICE
            fck_norm = instance.vehicle.fuel_cost_per_km / MAX_FUEL_COST
            vt_norm  = instance.vehicle.type_index / 3.0
            n_orders = max(len(instance.orders), 1)
            viol_rate = best.violations() / n_orders
            state = np.array([
                sp,
                np.clip(delta, -1.0, 1.0),
        ...
        n_remove = rng.randint(1, min(6, len(instance.orders)))
        prev_cost = current.cost()
 
        new_route, removed = destroy_op(current.route, orders_map, instance.vehicle, n_remove, rng)
        new_route = repair_op(new_route, removed, orders_map, instance.vehicle, rng)
        ...
        if mode == "ppo":
            ...
            # Pure greedy: accept only if strictly better
            if new_cost < prev_cost:
                current = candidate
    ...
    result   = check_route(best.route, orders_map, instance.vehicle)
    metrics  = compute_metrics(best.route, instance.vehicle)
...
def main():
    ...
    for inst in instances:
        for run in range(args.runs):
            ...
            # ── Augment ONCE so both solvers share identical vehicle/orders ──
            # Using a separate rng so augmentation seed is independent of the
            # operator-selection seed used inside run_once.
            aug_rng = random.Random(seed)
            shared_inst = augment_instance(inst, aug_rng)
 
            # travel_seed ensures A→B speed is identical for greedy and PPO.
            # We derive it from seed so it's also reproducible across reruns.
            travel_seed = seed + 999983  # large prime offset avoids collision
 
            g = run_once(shared_inst, "greedy", None,      args.max_iter, seed, travel_seed)
            p = run_once(shared_inst, "ppo",    ppo_model, args.max_iter, seed, travel_seed)
