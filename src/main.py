...
# # main.py
# # =======
# # Demo and inference runner for PPO-ALNS VRPTW.

# # Usage (from implementation_3.0/):
# #     python src/main.py                          # greedy only demo
# #     python src/main.py --use_ppo                # with trained PPO model
# #     python src/main.py --instance INST0005_...  # specific instance
# #     python src/main.py --start_time 09:30       # custom route start time
# # """
...
# # # ─────────────────────────────────────────────────────────────────────────────
# # # Pure ALNS (no PPO) — used as baseline
# # # ─────────────────────────────────────────────────────────────────────────────

# # def run_alns_greedy(instance, max_iter=100, seed=42):
...
# # def run_ppo_alns(instance, model_path, max_iter=100):
# #     from stable_baselines3 import PPO as SB3PPO
# #     try:
# #         model = SB3PPO.load(model_path)
# #     except Exception as e:
# #         print(f"[WARN] Could not load PPO model: {e}. Falling back to greedy.")
...
# # def print_results(instance, solution: Solution, start_time: str = "08:00"):
# #     orders_map = {o.order_id: o for o in instance.orders}
...
# #           f"{instance.vehicle.fuel_type} | {instance.vehicle.mileage} km/l")
# #     print(f"  ORDERS   : {len(instance.orders)}")
# #     print("="*60)

# #     print(f"\n{'Stop':<5} {'Type':<10} {'District':<22} {'ETA':>6} {'ETD':>6} {'Svc':>5}")
# #     print("-"*65)
# #     for s in sched:
# #         print(f"{sched.index(s)+1:<5} "
# #               f"{s['type']:<10} "
# #               f"{s['district']:<22} "
# #               f"{s['eta']:>6} "
# #               f"{s['etd']:>6} "
...
# #     print(f"  Carbon emission  : {metrics['carbon_emission_kg']} kg CO₂")
# #     print(f"  Fuel cost        : ₹{metrics['fuel_cost_inr']:.2f}")
# #     print(f"  Deadline violations : {result.deadline_violations}")
# #     print(f"  Total lateness   : {result.total_lateness} min")
# #     print(f"  Feasible         : {'✓ YES' if result.feasible else '✗ NO'}")
# #     print(f"  Objective cost   : {solution.cost():.2f}")
# #     print()

# #     # JSON output (ETA/ETD per node)
# #     output = {
...
# #         if not inst:
# #             print(f"Instance {args.instance} not found, using random.")
...
# #     """Extended version of print_results that also generates charts."""
# #     orders_map = {o.order_id: o for o in instance.orders}
# #     result  = check_route(solution.route, orders_map, instance.vehicle)
# #     metrics = compute_metrics(solution.route, instance.vehicle)
# #     sched   = format_schedule(solution.route, result.arrival_times,
...
# #           f"₹{instance.vehicle.fuel_price}/l | "
# #           f"{instance.vehicle.emission_factor} kg CO₂/km")
# #     print(f"  ORDERS   : {len(instance.orders)} | "
# #           f"Max weight: {instance.vehicle.max_weight} kg | "
# #           f"Max volume: {instance.vehicle.max_volume} m³")
# #     print(f"{'='*80}")
...
# #     print(f"  Carbon emission : {metrics['carbon_emission_kg']} kg CO₂")
# #     print(f"  Fuel cost       : ₹{metrics['fuel_cost_inr']:.2f}")
# #     print(f"  Weight util     : {round(sum(o.weight_kg for o in instance.orders)/instance.vehicle.max_weight*100,1)}%")
# #     print(f"  Violations      : {result.deadline_violations}")
# #     print(f"  Total lateness  : {result.total_lateness} min")
# #     print(f"  Feasible        : {'✓ YES' if result.feasible else '✗ NO'}")
# #     print(f"  Objective cost  : {solution.cost():.2f}")
